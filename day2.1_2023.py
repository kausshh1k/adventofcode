lines = '''Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
...
for line in lines:
    ...
    games[gid] = []
    picks = line[line.index(':') + 2 : ].split(';')
    for pick in picks:
        if invalid and invalid[-1] == gid: break
        obs = [0, 0, 0]
        balls = pick.split(',')
        for i in range(len(balls)): balls[i] = balls[i].split()
        for count, ball in balls:
            if ball == 'red' and int(count) > 12:
                invalid.append(gid)
                break
            elif ball == 'green' and int(count) > 13:
                invalid.append(gid)
                break
            elif ball == 'blue' and int(count) > 14:
                invalid.append(gid)
                break

print(sum(games) - sum(invalid))
# print(sum(games))